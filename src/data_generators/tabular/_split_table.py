...
def split_table(
    input_table: Table,
    out_table_count: int,
    min_output_columns: int,
    max_output_columns: int,
    *fixed_columns: str
) -> list[Table]:
    fixed_col_count = len(fixed_columns)
    if min_output_columns < fixed_col_count:
        raise ValueError(f"min columns too small ({min_output_columns}<{fixed_col_count})")
    if max_output_columns > len(input_table.columns):
        raise ValueError(f"max columns too large ({max_output_columns}>{len(input_table.columns)})")
    ...
    result: list[Table] = []
    current = out_table_count
    while current > 0:
        out_table_col_count = _compute_out_col_count(
            fixed_col_count, min_output_columns, max_output_columns
        ) - fixed_col_count
